...
使用 `QTimer` 驱动，每 8ms 触发一次更新。

| 动画 | 实现方式 | 效果 |
|------|----------|------|
| **呼吸效果** | `math.sin(time.time() * 1.5)` | 边框亮度和文字大小周期性变化 |
| **脉冲边框** | 亮度随 `_pulse_intensity` 变化 | 边框颜色在 190-225 之间变化 |
| **文字缩放** | 缩放因子 `1.0 + pulse * 0.04` | 百分比数字轻微缩放 |
...
所有动画效果（呼吸、脉冲、闪烁）共用一个 8ms 的 `QTimer`，减少资源消耗。
